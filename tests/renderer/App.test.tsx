...
// The route test owns shell navigation only. Excalidraw itself is exercised
// through its focused scene tests and packaged Electron test.
vi.mock('@/features/canvas/canvas-workspace', () => ({
  CanvasWorkspace: () => (
    <div>
      <aside aria-label="Canvas item library">
        <input type="search" aria-label="Search Canvas items" />
      </aside>
      <main aria-label="Canvas workspace" />
    </div>
  )
}))
...
describe('App', () => {
  ...
  it('opens backup settings without exposing raw data-folder actions in the app shell', async () => {
    ...
    const semanticModelPanel = screen.getByRole('region', {
      name: 'Semantic model availability'
    })
    expect(within(semanticModelPanel).getByText('Universal Sentence Encoder Lite v1')).toBeVisible()
    expect(within(semanticModelPanel).getByText('Included with OnMove')).toBeVisible()
    expect(within(semanticModelPanel).getByText('27.1 MiB')).toBeVisible()
    expect(within(semanticModelPanel).getByText('Available offline')).toBeVisible()
    expect(within(semanticModelPanel).getByText('Updated with the app')).toBeVisible()
    expect(within(semanticModelPanel).getByText(/never download at runtime/)).toBeVisible()
    expect(within(semanticModelPanel).getByText(/derived embeddings persist separately/))
      .toBeVisible()
    expect(within(semanticModelPanel).getByText(/no model-cache eviction control/)).toBeVisible()
    expect(within(semanticModelPanel).getByText(/Clearing OnMove user data is not offered here/))
      .toBeVisible()
    ...
    expect(within(retrievalStatusPanel).getByText('Loading the local semantic model'))
      .toBeVisible()
    expect(within(retrievalStatusPanel).getByText(/initializing its bundled model locally/))
      .toBeVisible()
