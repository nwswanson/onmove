...
const REFERENCE_COLUMNS = `canvas_id, shape_id, entity_type, entity_id,
...
function normalizeShapeId(value: string): string {
  if (typeof value !== 'string' || !/^shape:[A-Za-z0-9_-]{1,153}$/.test(value)) {
    throw new ModelValidationError('Canvas shape id must be a canonical TLDraw shape id')
  }
  return value
}
 ...
 * domain records. TLDraw geometry remains opaque; entity meaning never does.
 */
export class CanvasRepository {
        ...
        if (reference.missing_since === null) {
          this.database.run(
            `UPDATE canvas_entity_references
             ...
             WHERE canvas_id = ? AND shape_id = ?`,
            [now, now, id, reference.shape_id]
          )
  ...
  addEntityReference(
    ...
    const shapeId = normalizeShapeId(input.shapeId)
    ...
    this.database.run(
      `INSERT INTO canvas_entity_references (
         canvas_id, shape_id, entity_type, entity_id, cached_title,
  ...
  saveDocument(canvasId: number, input: SaveCanvasDocumentInput): CanvasSummarySnapshot {
    ...
    if (!Array.isArray(input.entityShapeIds)) {
      throw new ModelValidationError('Canvas entity shape ids must be an array')
    }
    const shapeIds = input.entityShapeIds.map(normalizeShapeId)
    if (new Set(shapeIds).size !== shapeIds.length) {
      throw new ModelValidationError('Canvas entity shape ids must be unique')
    }
    const now = timestamp()
      ...
      if (shapeIds.length === 0) {
        ...
        this.database.run(
          `DELETE FROM canvas_entity_references
           WHERE canvas_id = ? AND shape_id NOT IN (${placeholders})`,
          [canvasId, ...shapeIds]
        )
      }
    })
