...
followed by `_` or `-` is rejected rather than partially matched. Identity is canonical Unicode
lowercase: `@Launch` and `@launch` both appear as `@launch` in Tags. Stored user text retains its
original spelling.
...
node metadata can never appear as a use or snippet. A use is one canonical tag per field. Repeating
`@launch` in the same title, Goal, Update, Todo, or Note does not create duplicate backreferences;
the first occurrence supplies that field's snippet.
...
- `listTags()` returns lowercase canonical names with total and effectively-sensitive field counts.
- `listTagUses(name)` validates and canonicalizes one name and returns only that name's field uses.

Each field use contains a short normalized plain-text snippet, a typed source/field reference, its
...
effective hierarchy sensitivity. The feature presenter removes sensitive field uses and any tag
